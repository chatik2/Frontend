...
class App extends Component {
  ...
  addTrack() {
      ...
      axios.post('http://138.197.150.20/new_msg', data, headers);
  ...
  updateUser() {
    ...
    axios.post('http://138.197.150.20/new_name', data1);
...
//
    if (!localStorage.getItem('KeyForReactApp')) {
        axios.get('http://138.197.150.20/new_user').then(response => { 
...
//
//
    axios.get('http://138.197.150.20/allusers?uid=157', headers)
    .then(response => {
...
//
    axios.get('http://138.197.150.20/allmsgs?uid=157', headers)
    .then(response => {
      console.log(response);
